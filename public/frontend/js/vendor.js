...
siteApp.filter('unsafe', function($sce) { return $sce.trustAsHtml; });


siteApp.filter('trustedUrl', ['$sce', function ($sce) {
    return function(url) {
...
siteApp.run(['$rootScope', function($rootScope) {
    $rootScope.customGA = function(event_name, optional_params ){
        gtag('event', event_name, optional_params);
    };
}]);
...
siteApp.controller('blogController', [ '$scope', '$http', function($scope, $http) {
    $scope.list = [];
    $scope.hasMorePost = false;
    $scope.blogHeight;
    $scope.width = $(window).innerWidth();
    if($(".image-wrapper:has(img)").length < 0){

    }else{
        if($scope.width >= 1281 && $scope.width <= 1400){
            $scope.blogHeight =240;
        }
        else if($scope.width >= 1025 && $scope.width <= 1280){
            $scope.blogHeight = 220;
        }
        else if($scope.width >= 960 && $scope.width <= 1024 || $scope.width >= 400 && $scope.width <= 639 ){
            $scope.blogHeight =180;
        }
        else if($scope.width >= 640 && $scope.width <= 959){
            $scope.blogHeight =155;
        }
        else if($scope.width <= 399){
            $scope.blogHeight =150;
        }
        else {
            $scope.blogHeight = 253;
        }
    }
    $http({
        method: 'POST',
        url: APP.baseUrl + 'blog/api.php?action=recent-posts',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded'
        }
    }).then(function (response) {
        if (response.status === 200) {
            $scope.list[$scope.list.length] = response.data.posts;
            $scope.hasMorePost = response.data.posts && response.data.posts.length;
            setTimeout(function () {
                $('.blog-container').addClass('');
            }, 1500);
        }
    ...
    $scope.loadMore = function() {
        ...
        // if video is loaded initially
        var iframe_height = $('.image-wrapper').height();
        if($(".image-wrapper:has(img)").length > 0){
            $scope.blogHeight = iframe_height - 3;
        }else{
            if($scope.width >= 1281 && $scope.width <= 1400){
                $scope.blogHeight =240;
            }
            ...
            else if($scope.width >= 300 && $scope.width <= 639){
                $scope.blogHeight =150;
            }
            else {
                $scope.blogHeight = 253;
        ...
        $http({
            method: 'POST',
            url: APP.baseUrl + 'blog/api.php?action=load-more&offset=' + offset + '&height='+iframe_height,
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded'
            }
        }).then(function (response) {
            if (response.status === 200) {
                ...
                // $('#post-video-parent').css('min-height',iframe_height);
                $scope.list[$scope.list.length] = response.data.posts;
                $scope.hasMorePost = response.data.posts && response.data.posts.length;
            }
...
siteApp.controller('careerController', [ '$scope', '$http', function($scope, $http) {
    $scope.entry = {};
    $scope.errors = {};
    $scope.corrects = {};
    $scope.isError = false;
    ...
    $scope.isEmailSubmitting = false;
    $scope.registerTokenUpdateCount = 0;
    $scope.response = "";
    ...
    $scope.restrictInNum = function restrictInNum() {
        $scope.entry.age = $scope.entry.age.replace(/[^0-9+]/g, '');
    };
    $scope.restrictInName = function restrictInName() {
        $scope.entry.name = $scope.entry.name.replace(/[^a-zA-Z,.' -]/, '');
    };
    $scope.submit = function () {
        ...
        angular.forEach($scope.entry.interest, function (value, key) {
            if(value === true) {
                if(interest !== '') {
                    interest = interest + ','
                }
        ...
        $scope.entry.interests = interest;

        var file_input = document.getElementById('resume_upload');

        $scope.entry.resume_upload = file_input.files[0];
        ...
        $http({
            method: 'POST',
            url: APP.baseUrl + 'workshop/career-entry',
            data: formData,
            headers: {
                'Content-Type': undefined,
            ...
            if (response.status === 200) {
                ...
                $scope.errors['general'] = [response.message];
            }
            $scope.isSubmitting = false;
            $scope.isLoaded = true;
        }, function (error) {
            if (error.status === 422) {
                ...
                $scope.errors['general'] = ['Resume file size is too large, kindly upload file having less than 5mb size'];
            } else {
                $scope.errors['general'] = ['An error occurred, please try again later'];
            }
            $scope.isSubmitting = false;
            $scope.isLoaded = true;
        });
...
siteApp.controller('contactController', [ '$scope', '$http', function($scope, $http) {
    $scope.user = {};
    $scope.errors = {};
    $scope.corrects = {};
    $scope.isError = false;
    ...
    $scope.restrictInNum = function restrictInNum() {
        $scope.user.phone = $scope.user.phone.replace(/[^0-9+]/g, '');
    };

    $scope.restrictInFname = function restrictInFname() {
        $scope.user.first_name = $scope.user.first_name.replace(/[^a-zA-Z,.' -]/, '');
    };

    $scope.restrictInLname = function restrictInLname() {
        $scope.user.last_name = $scope.user.last_name.replace(/[^a-zA-Z,.' -]/, '');
    };
    ...
    $scope.submit = function () {
        ...
        if(APP.env === 'production'){
            var optional_params = {
                'send_to': 'AW-774381738/0xYlCLXsr5EBEKrBoPEC'
            };

            $scope.customGA('conversion', optional_params);
        }
        ...
        $http({
            ...
            if (response.status === 200) {
                $('.success-notification-popup').removeClass('hide-popup');
                $scope.response = response.data.message;
                setTimeout(function () {
                    $('.success-notification-popup').addClass('hide-popup');
                }, 5000);
                $scope.user = {};
            } else {
                if (response.csrfToken && $scope.registerTokenUpdateCount < 3) {
                    APP.csrfToken = response.csrfToken;
                    $scope.registerTokenUpdateCount++;
                    setTimeout(function () {
                        $scope.isSubmitting = false;
                        $scope.isLoaded = true;
                        $scope.submit();
...
siteApp.controller('entriesController', [ '$scope', '$http', function($scope, $http) {
    $scope.entry = {};
    $scope.errors = {};
    $scope.corrects = {};
    $scope.isError = false;
    ...
    $scope.submit = function () {
        ...
        $http({
            ...
            if (response.status === 200) {
                $scope.response = response.data.message;
                if(response.data.status === 'success') {
                    window.location.href = APP.baseUrl + 'workshop/thanks';
                }
            ...
            $('html, body').animate({
                scrollTop: $('.enroll-form').offset().top
            }, 100);
            $scope.isSubmitting = false;
            $scope.isLoaded = true;
        });
