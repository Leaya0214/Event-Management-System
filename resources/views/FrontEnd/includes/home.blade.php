...
@section('content')
<!-- Font Imports -->
<link href="https://fonts.googleapis.com/css2?family=Carme&family=Dongle:wght@300&family=Poppins:wght@300&display=swap" rel="stylesheet">
...
<div class="article-section">
    ...
    <p class="section-content">
        Bridal Harmony is a team of experienced professional photographers, cinematographers and photo-book experts who are dedicated to creating stunning, authentic stories of people's live.
    </p>
</div>
        ...
        @foreach ($services as $service)
            <div class="article figure">
                <div class="article-image article1">
                    <a href="">
                        <img data-lazy="{{ asset('backend/service/' . $service->image_video) }}"
                             type="image"
                             alt="Bridal Harmony - {{ $service->title }}"
                             src="#">
                    </a>
                </div>
                ...
                <div class="read-more">
                    <!-- Read More functionality can be added here -->
                </div>
            </div>
        @endforeach
    </div>
...
<section class="article-section animatedParent animateOnce">
    <h1 class="animated fadeInDownShort" style="padding-top:20px">
        <div class="straight-border"></div>
        RECENT WORK
        <div class="straight-border"></div>
    </h1>
    <p class="section-content">
        Take a peek at our recent stunning wedding photography album, which is loaded with wonderful moments and memories.
    </p>
        ...
        @foreach ($portfolios as $portfolio)
            ...
            <div class="article figure">
                @if($portrait)
                    <div class="article-image article1">
                        <a href="{{ url('our_work_view/' . $portfolio->id . '/' . $portfolio->title )}}">
                            <img data-lazy="{{ asset('backend/portfolio_gallery/' . $portrait->image) }}"
                                 type="image"
                                 alt="Bridal Harmony - {{ $portfolio->title }}"
                                 src="#"
                                 height="auto"
                                 width="auto">
                        </a>
                ...
                @else
                    <div class="article-image article1">
                        <a href="{{ url('our_work_view/' . $portfolio->id . '/' . $portfolio->title )}}">
                            <img data-lazy="{{ asset('backend/portfolio/' . $portfolio->image) }}"
                                 type="image"
                                 alt="Bridal Harmony - {{ $portfolio->title }}"
                                 src="#"
                                 height="auto"
                    ...
                    <div class="video-wrapper">
                        <a href="" data-url="{{ getYoutubeEmbedUrl($video->video) }}" tabindex="-1">
                            <img class="responsive-image"
                                 src="{{ asset('backend/portfolio/' . $video->image) }}"
                                 alt="Bridal Harmony - {{ $video->title }}">
                            <div class="play-icon">
                                <img src="{{ asset('frontend/images/play-icon.png') }}"
                                     alt="Bridal Harmony - video play icon">
                            </div>
                        </a>
                    </div>
            ...
            <div class="article figure">
                <div class="article-image article1">
                    <a href="" target="_blank">
                        <img data-lazy="{{ asset('backend/blog/' . $blog->image) }}"
                             type="image"
                             alt="Bridal Harmony - {{ $blog->title }}"
                             src="#">
                    </a>
                </div>
                <div class="figcaption">
...
<div class="hide">
    ...
    @foreach ($services as $s_data)
        <div class="modal-box blog-id-{{ $s_data->id }}"
             id="work-with-us-{{ $s_data->id }}"
             data-ng-controller="careerController">
            <div class="modal-info">
                <div class="modal-contents">
                    <h1>{{ $s_data->title }}</h1>
                    <img src="{{ asset('backend/service/' . $s_data->image_video) }}"
                         alt="{{ $s_data->title }}"
                         style="width: 100%">
                    <div class="description">
                        {!! $s_data->description !!}
    ...
    @foreach ($blogs as $b_data)
        @php
            $images = DB::table('blog_images')
                ...
                ->get();
        @endphp
        <div class="modal-box blog-id-{{ $b_data->id }}"
             id="work-with-us-{{ $b_data->id }}"
             data-ng-controller="careerController">
            <div class="modal-info">
                <div class="modal-contents">
                    <h1>{{ $b_data->title }}</h1>
                    <img src="{{ asset('backend/blog/' . $b_data->image) }}"
                         alt="{{ $b_data->title }}"
                         style="width: 100%">
                    <div class="description">
                        {!! $b_data->description !!}
                    ...
                    <div class="gallery" style="padding-left: 11rem">
                        @foreach ($images as $image)
                            <img src="{{ asset('backend/blog_gallery/' . $image->image) }}"
                                 alt="Gallery Image"
                                 height="250"
                                 style="padding:3% 4% 3% 1%">
                        @endforeach
                    </div>
                </div>
            </div>
