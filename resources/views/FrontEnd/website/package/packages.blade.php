            ...
            <div class="row">
                        ...
                        <li class="list-group-item mb-2 first-li" style="font-size:15px"><button onclick="location.reload()"
                         ...
                         @foreach ($package_category as $category)
                            <li class="list-group-item mb-2  first-li" style="font-size:15px"><button data-group="category"
                                    class="category-button" data-id="{{ $category->id }}" data-name ="{{ $category->category_name }}" data-branch="{{$branch->id}}" data-category="{{ $category->id }}">
                ...
                <div class="col-md-9">
                    {{-- <div class="mb-4 text-center ">
                        <button class="btn btn-lg btn-secondary h-10 px-4 py-2 selected type-button" data-group="type"
                            data-type="all" value="500">
                            ALL Type
                        </button>
                        @foreach ($package_type as $type)
                            <button data-group="type" class="btn btn-lg h-10 px-4 py-2 btn-secondary type-button "
                                data-type="{{ $type->id }}">
...
<script>
    $(document).ready(function () {
        ...
        $(".category-button").click(function () {
            ...
            var link = "{{ route('category_package', ['id' => ':category_id','branch'=>':package_branch_id', 'name' => ':category_name']) }}";
            link = link.replace(':category_id', categoryId).replace(':package_branch_id', branchId).replace(':category_name', categoryName);
            var tempInput = $("<input>");
    ...
    <script>
        function loadModalContent(itemId) {
            // Fetch modal content using AJAX
            $.ajax({
